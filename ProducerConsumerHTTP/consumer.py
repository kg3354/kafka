...
base_url = 'http://kafka.hsrn.nyu.edu'

# Update these variables with your actual username and password
username = 'username'   #replaced by actual username from the htpasswd process
password = 'aSimplePassword' #replaced by actual password from the htpasswd process
...
def create_consumer(group_id, instance_id, format="json"):
    ...
    response = requests.post(url, headers=headers, data=json.dumps(body), auth=HTTPBasicAuth(username, password))
